...
public:
    ...
    void read_packet_header()
    {
        ...
        buf_ = fusionary::read(hdr, buf_);

        std::cout << "Protocol version " << std::showbase << std::hex << hdr.version.value()
                  << std::endl
                  << "Packet sequence  " << std::showbase << std::hex << hdr.packet_sequence.value()
                  << std::endl;
    }
