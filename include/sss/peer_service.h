 ...
 * Integrates an sss::server to handle incoming connections
...
class peer_service
{
    ...
    server server_;                                     // To accept incoming streams
    const std::string service_name_;                    // Name of service we provide
    const std::string protocol_name_;                   // Name of protocol we support

    std::map<peer_identity, stream*> out;                     // Outgoing streams by host ID
    std::map<peer_identity, std::unordered_map<stream*>> in;  // Incoming streams by host ID
...
public:
     ...
     */
    stream *connect_to_peer(peer_identity eid);
     ...
     */
    inline stream *out_stream(peer_identity eid) {
        return out[eid];
    }
     ...
     */
    inline std::unordered_map<stream*> in_streams(peer_identity eid) {
        return in[eid];
    }
    ...
    boost::signals2::signal<void(peer_identity const&)>  on_peer_status_changed;
};
