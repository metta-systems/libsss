...
{
    std::shared_ptr<host> host_;    // Host to which this key is attached.
    async::timer expiration_timer_; // Expiration timer for key.
...
public:
    shortterm_key_t(std::shared_ptr<host> host);
    ~shortterm_key_t();
};
